...
## try reading in data functionally. hopefully won't run multiple times in pool
...
def read_data(order):
    ...
    dat = pd.read_csv(f'inputs\\df_order{order}.csv').values
...
def rust_mp(df,results,n_processes,order):
    ## method
    results['Method'] = 3
    ...
    results['Elapsed (s)'] = toc - tic
    ## number of observations
    results['Size (N)'] = df.shape[0]
    # return results
    print(results)
    results.to_csv(f'outputs\\results3_order{order}_p{n_processes}.csv',index=False)
...
if __name__ == '__main__':
    ...
    dat, order_choice = read_data(order=8)

    #######################
    ## define results df ##
    #######################

    results_3 = pd.DataFrame(np.zeros((1,3)),columns=['Method','Size (N)', 'Elapsed (s)'])

    ###########################################################
    ## call multiprocess likelihood computation for dat_list ##
    ###########################################################

    # rust_mp(dat,results_3,n_processes=2,order=order_choice)
    # rust_mp(dat,results_3,n_processes=4,order=order_choice)
    # rust_mp(dat,results_3,n_processes=8,order=order_choice)
    rust_mp(dat,results_3,n_processes=16,order=order_choice)
